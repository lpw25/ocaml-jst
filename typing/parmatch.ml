...
| {pat_desc = Tpat_tuple omegas} ->
    ...
    make_pat (Tpat_tuple args) q.pat_type q.pat_mode q.pat_effs q.pat_env::rest
      ...
      (Tpat_record
         (List.map2 (fun (lid, lbl,_) arg ->
           ...
           then
             ...
             lid, lbl, arg)
            omegas args, closed))
      q.pat_type q.pat_mode q.pat_effs q.pat_env::
    rest
...
| {pat_desc = Tpat_variant (l, omega, row)} ->
    ...
    in
    make_pat
      (Tpat_variant (l, arg, row)) q.pat_type q.pat_mode q.pat_effs q.pat_env::
    rest
| {pat_desc = Tpat_lazy _omega} ->
    begin match r with
      arg::rest ->
        make_pat (Tpat_lazy arg) q.pat_type q.pat_mode
          q.pat_effs q.pat_env::rest
    | _ -> fatal_error "Parmatch.do_set_args (lazy)"
    end
| {pat_desc = Tpat_array omegas} ->
    let args,rest = read_args omegas r in
    make_pat
      (Tpat_array args) q.pat_type q.pat_mode q.pat_effs q.pat_env::
    rest
...
let orify x y =
  make_pat (Tpat_or (x, y, None)) x.pat_type x.pat_mode x.pat_effs x.pat_env
...
let build_other_constant proj make first next p env =
  ...
  let rec try_const i =
    ...
    else make_pat (make i) p.pat_type p.pat_mode p.pat_effs p.pat_env
  in try_const first
          ...
          let tags =
            ...
            let make_other_pat tag const =
              ...
              make_pat (Tpat_variant(tag, arg, cstr_row))
                d.pat_type d.pat_mode d.pat_effs d.pat_env
            in
            let row = type_row () in
            begin match
              List.fold_left
          ...
          let rec try_arrays l =
            if List.mem l all_lengths then try_arrays (l+1)
              ...
              make_pat (Tpat_array (omegas l))
                d.pat_type d.pat_mode d.pat_effs d.pat_env in
          try_arrays 0
      | _ -> Patterns.omega
...
| Tpat_tuple ps, Tpat_tuple qs ->
    ...
    make_pat (Tpat_tuple rs) p.pat_type p.pat_mode p.pat_effs p.pat_env
| Tpat_lazy p, Tpat_lazy q ->
    let r = lub p q in
    make_pat (Tpat_lazy r) p.pat_type p.pat_mode p.pat_effs p.pat_env
| Tpat_construct (lid, c1,ps1), Tpat_construct (_,c2,ps2)
      when  Types.equal_tag c1.cstr_tag c2.cstr_tag  ->
        ...
        make_pat (Tpat_construct (lid, c1,rs))
          p.pat_type p.pat_mode p.pat_effs p.pat_env
